...
def main() -> int:
    ...
    # Score audio this script did not synthesise. The point is cross-architecture
    # comparison: a VITS system writes <clip_id>.wav into a directory, and every
    # metric is then computed by THIS code rather than a second implementation --
    # including SECS, which keeps using XTTS's speaker encoder so the number
    # means the same thing for both systems. Without that, comparing two
    # architectures really compares two metric implementations.
    ap.add_argument("--synth-dir", default=None, metavar="DIR",
                    help="score existing <clip_id>.wav files instead of synthesising")
    args = ap.parse_args()
    ...
    synth_dir = Path(args.synth_dir).resolve() if args.synth_dir else None
    if synth_dir:
        missing = [it["clip_id"] for it in items
                   if not (synth_dir / f"{it['clip_id']}.wav").is_file()]
        if missing:
            print(f"{len(missing)} of {len(items)} clips missing from "
                  f"{synth_dir}; first: {missing[:3]}", file=sys.stderr)
            return 2
        print(f"audio      : pre-synthesised, from {synth_dir}")

    print("\nreading audio" if synth_dir else "\nsynthesising")
    rows = []
    for i, it in enumerate(items, 1):
        ...
        if synth_dir:
            # Already generated by another system. Conditioning latents are what
            # XTTS synthesises FROM, so they are not computed here -- only the
            # scoring below runs. RTF belongs to whatever produced the audio and
            # is left unset rather than guessed; benchmark.py is where speed is
            # measured, for either architecture.
            syn_path = synth_dir / f"{it['clip_id']}.wav"
            _w, _sr = torchaudio.load(str(syn_path))
            if _w.shape[0] > 1:                      # scoring assumes mono
                _w = _w.mean(0, keepdim=True)
            syn_sec, elapsed = _w.shape[-1] / _sr, None
        else:
        ...
        rows.append({**it, "synth_wav": str(syn_path), "cond_wav": str(cond),
                     ...
                     "rtf": None if elapsed is None
                            else elapsed / max(syn_sec, 1e-6)})
